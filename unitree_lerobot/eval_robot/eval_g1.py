...
def eval_policy(
    ...
    arm_ctrl, arm_ik, ee_shared_mem, arm_dof, ee_dof, full_joint_names = (
        robot_interface[key]
        for key in ["arm_ctrl", "arm_ik", "ee_shared_mem", "arm_dof", "ee_dof", "full_joint_names"]
    )
    ...
    if user_input.lower() == "s":
        ...
        while True:
            ...
            full_state = np.concatenate([current_arm_q, ee_q])
            state_tensor = torch.from_numpy(adapter.to_policy(full_state)).float()
